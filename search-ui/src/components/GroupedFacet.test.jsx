...
describe('GroupedFacet', () => {
  it('renders one card per group', () => {
    ...
    expect(screen.getByRole('button', { name: /Adjudicado/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Desierto/i })).toBeInTheDocument()
  ...
  it('emits the full code set of a group when toggled on', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Adjudicado/i }))
  ...
  it('shows the summed count of a group', () => {
    ...
    expect(screen.getByRole('button', { name: /Adjudicado/i })).toHaveTextContent('11')
  })
  ...
  it('marks a card active via aria-pressed when all its codes are selected', () => {
    ...
    expect(screen.getByRole('button', { name: /Adjudicado/i })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: /Desierto/i })).toHaveAttribute('aria-pressed', 'false')
  })
})
